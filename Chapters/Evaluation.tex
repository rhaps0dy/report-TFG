...
\includegraphics[scale=1]{img/sarsa/#1.pdf}
\end{center}
\caption{#2\label{fig:#1}}
\end{figure}
}

\drawtraining{without_options}{The reward per episode, averaged every 1000
...
episodes.}
\drawtraining{with_options}{The reward per episode, averaged every 1000
episodes, as the training progresses. Options are used. Blue uses
annealing, red does not. Darker lines, with higher value,
do not include the shaping reward $F(\cdot)$; lighter lines show the reward as
the agent experiences it. $y$ axis is reward, $x$ axis is thousands of
episodes.}
